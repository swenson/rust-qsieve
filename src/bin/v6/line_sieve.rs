use crate::v6::{line_sieve_double, poly};
...
pub fn split_line_sieve<const L1_SHIFT: u32>(
    ...
    let sqrt_num: BigInt = num.sqrt() + 1;
    ...
    // for (p, s0, s1) in roots.iter().take(medium_prime_offset) {
    //     let p = *p as isize;
    //     let sqrtp = (sqrt_num.clone() % p).to_isize().unwrap();
    //     let logp = p.ilog2() as u8;
    //     let mut i0 = (*s0 as isize - sqrtp) % p;
    //     let i0 = i0 as u64;
    //     let mut i1 = (*s1 as isize - sqrtp) % p;
    //     let i1 = i1 as u64;
    //     line_sieve(area, sieve, p, 0, logp, vec![i0.min(i1), i0.max(i1)]);
    //     // println!(
    //     //     "line_sieve_double {} {} {} {} {} {} {}",
    //     //     0, i0, i1, logp, p, l1_cache_size, area
    //     // );
    //     // assert!(
    //     //     (((i0 + 0 + sqrt_num.clone()) * (i0 + 0 + sqrt_num.clone()) - num.clone()) % p)
    //     //         .is_zero()
    //     // );
    //     // unsafe {
    //     //     line_sieve_double(
    //     //         sieve.as_mut_ptr().offset(0),
    //     //         i0.min(i1),
    //     //         i0.max(i1),
    //     //         logp,
    //     //         p,
