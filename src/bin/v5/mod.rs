...
pub fn main_fun(
    ...
    for p in primes.iter() {
        ...
        let logp = (*p as f64).log2().ceil() as u8;
...
#[inline(always)]
fn line_sieve(area: usize, sieve: &mut Vec<u8>, p: isize, sqrtp: isize, logp: u8, roots: Vec<u64>) {
    if roots.len() == 2 {
        let i0 = (roots[0] as isize - sqrtp).rem_euclid(p);
        let i1 = (roots[1] as isize - sqrtp).rem_euclid(p);
