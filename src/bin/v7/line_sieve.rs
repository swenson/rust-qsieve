use crate::v7::{line_sieve_double, poly};
...
pub fn split_line_sieve<const L1_SHIFT: u32>(
        ...
        .enumerate()
    {
        ...
        for j in (0..l1_cache_size as usize).step_by(recalculate_every) {
            let s = BigInt::from(j as isize + start + sqrt_num.clone())
                * BigInt::from(j as isize + start + sqrt_num.clone())
                - num.clone();
            let cutoff = s.to_f64().unwrap().log2().ceil() as u8 - fudge;
